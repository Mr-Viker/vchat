<template>
  <section class="page chat-list-page">
    ...
    <div class="page-bd">
      <mt-cell-swipe :title="item.username" :label="item.content_type == 1 ? '[图片]' : item.content" :to="'/Chat?id=' + item.uid + '&num=' + item.new_chat_num" class='v-cell v-cell-2' v-for='(item, index) in chatList' :key='item.uid' :right="[{content: '删 除', style: { background: '#f44336', color: '#fff', width: '.7rem' }, handler: () => delChatList(item)}]">
